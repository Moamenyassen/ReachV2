...
import { ReachPricing, ModernOSLayout, PasswordChangeModal } from './components';
...
// Password / tokens must never be stored client-side — XSS surfaces them all.
const SENSITIVE_USER_FIELDS = ['password', 'passwordHash', 'token', 'accessToken', 'refreshToken', 'apiKey', 'secret'] as const;
const persistUserSafe = (user: any) => {
  if (!user) {
    localStorage.removeItem('rg_v2_user');
    return;
  }
  const safe: any = { ...user };
  for (const k of SENSITIVE_USER_FIELDS) delete safe[k];
  try { localStorage.setItem('rg_v2_user', JSON.stringify(safe)); } catch { /* quota */ }
};
...
const App: React.FC = () => {
  ...
  const [isValidatingSession, setIsValidatingSession] = useState<boolean>(() => {
    // Only need to validate if we restored a user from localStorage
    try { return !!localStorage.getItem('rg_v2_user'); } catch { return false; }
  });

  // --- SESSION VALIDATION ON MOUNT ---
  // Guard against deleted/disabled users whose localStorage session was never cleared
  // Fetches fresh app_users row and applies it to state. Returns the row or null.
  const refreshUserFromDB = useCallback(async (userId: string, userEmail?: string): Promise<any | null> => {
    // Try by primary key first (most reliable)
    let { data: dbUser } = await supabase.from('app_users').select('*').eq('id', userId).single();
    // Fallback: try by email (needed when auth_user_id is unlinked)
    if (!dbUser && userEmail) {
      const { data: byEmail } = await supabase.from('app_users').select('*').eq('email', userEmail).single();
      dbUser = byEmail;
    }
    ...
    const freshUser: User = {
      ...
      branchIds: dbUser.branch_ids || [],
      routeIds: dbUser.route_ids || [],
      regionIds: dbUser.region_ids || [],
      repCodes: dbUser.rep_codes || [],
      firstName: dbUser.first_name,
      lastName: dbUser.last_name,
      email: dbUser.email,
      phone: dbUser.phone,
      isRegisteredCustomer: dbUser.is_registered_customer,
    };
    setCurrentUser(freshUser);
    persistUserSafe(freshUser);

    if (freshUser.companyId) {
      const { data: companyData } = await supabase.from('companies').select('*').eq('id', freshUser.companyId).single();
      if (companyData) {
        const company: Company = {
          ...
          maxRoutes: companyData.max_routes,
          maxCustomers: companyData.max_customers,
          maxScannerCap: companyData.max_scanner_cap,
          isActive: companyData.is_active,
          createdAt: companyData.created_at,
          expirationDate: companyData.expiration_date,
          features: companyData.features,
          adminUsername: companyData.admin_username,
          settings: companyData.settings,
        };
        setCurrentCompany(company);
        localStorage.setItem('rg_v2_company', JSON.stringify(company));
        updateSupabaseHeaders({ 'x-company-id': companyData.id });
      }
    }
    return dbUser;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const savedUser = (() => { try { const s = localStorage.getItem('rg_v2_user'); return s ? JSON.parse(s) : null; } catch { return null; } })();

        if (session?.user) {
          // Resolve user by email (auth_user_id may not be set for users registered via registerGlobalUser)
          const email = session.user.email || savedUser?.email;
          const dbUser = await refreshUserFromDB(savedUser?.id || '', email);
          if (cancelled) return;
          if (!dbUser) {
            handleLogout();
            return;
          }
        } else if (savedUser?.id && savedUser.username !== 'sysadmin') {
          // No auth session — validate the stored user still exists in app_users
          const dbUser = await refreshUserFromDB(savedUser.id, savedUser.email);
          if (cancelled) return;
          if (!dbUser) {
            handleLogout();
            return;
        ...
        // Network error — fail safe (don't log out offline users)
      } finally {
  ...
  const handleSubscribe = async (planId: string, billingCycle: 'monthly' | 'yearly', licenseCount: number = 1) => {
    ...
    try {
      ...
      // Redirect to App Dashboard (icon launcher)
      ...
      {isLimbo && view !== ViewMode.REFERRAL_HUB && view !== ViewMode.PRICING && (
        licenseRequestStatus === 'LICENSE_REQUEST' ? (
          <PendingLicenseScreen
            ...
            onLogout={handleLogout}
          />
        ) : licenseRequestStatus === 'provisioned' || licenseRequestStatus === 'PROVISIONED' ? (
          <div className="fixed inset-0 z-[9000] bg-[#0f172a] flex flex-col items-center justify-center text-center px-6 animate-in fade-in duration-500">
            <div className="relative w-24 h-24 mb-8">
              <div className="absolute inset-0 border-t-[3px] border-emerald-500 rounded-full animate-[spin_1s_linear_infinite]"></div>
              <div className="absolute inset-2 border-r-[3px] border-cyan-500 rounded-full animate-[spin_1.5s_linear_infinite]"></div>
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-3xl">🎉</span>
              </div>
            </div>
            <h2 className="text-3xl font-black text-white mb-3">Your License is Activated!</h2>
            <p className="text-slate-400 max-w-md mb-8 text-base leading-relaxed">
              Welcome aboard, {currentUser?.firstName || currentUser?.username}! Your workspace is ready.
              Click below to enter the dashboard.
            </p>
            <button
              onClick={async () => {
                try {
                  let dbRow = await refreshUserFromDB(currentUser!.id!, currentUser?.email);

                  // Self-healing: if no company_id (user was activated before linkage fix),
                  // find the company by lead's company_name and link the user.
                  if (!dbRow?.company_id && currentUser?.email) {
                    ...
                    if (lead?.status === 'provisioned' && lead.company_name) {
                      ...
                      if (matchedCompany) {
                        await supabase
                          .from('app_users')
                          .update({ company_id: matchedCompany.id, role: UserRole.ADMIN })
                          .eq('id', currentUser!.id!);
                        dbRow = await refreshUserFromDB(currentUser!.id!, currentUser.email);
                      }
                    }
                  }
                  ...
                  if (dbRow?.company_id) {
                    ...
                    errorToast('Workspace setup incomplete', 'Please contact support — your account could not be linked to a company.');
                  }
                } catch (e: any) {
                  console.error('Go to Dashboard error:', e);
                  errorToast('Could not load workspace', e?.message || 'Unknown error');
                }
              }}
              className="px-8 py-3 bg-gradient-to-r from-emerald-500 to-cyan-500 text-white font-bold rounded-xl hover:opacity-90 transition-all text-lg shadow-lg shadow-emerald-500/20"
            >
              Go to Dashboard →
            </button>
            <button
