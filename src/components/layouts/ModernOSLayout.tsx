...
const SortableItem = ({ id, app, iconDims, textSize, controlProps, onClick, brandTheme, onOpenMenu, menuOpen, menuSlot }: any) => {
    const {
        attributes,
        listeners,
        setNodeRef,
    ...
    return (
        <div
            ...
            {...attributes}
            {...listeners}
            className="flex flex-col items-center gap-4 cursor-grab active:cursor-grabbing touch-none outline-none relative"
            onClick={(e) => {
                if (!isDragging) {
        ...
        >
            {/* Popover overlay (kept outside the clipped icon wrapper) */}
            {menuOpen && menuSlot}
            <div
            ...
            >
                ...
                {/* ⋯ trigger — opens the Move-to-group menu */}
                {onOpenMenu && (
                    <button
                        onClick={(e) => { e.stopPropagation(); onOpenMenu(); }}
                        onPointerDown={(e) => e.stopPropagation()}
                        title="Options"
                        className="absolute top-1.5 right-1.5 w-6 h-6 rounded-full bg-black/40 hover:bg-black/70 text-white opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm border border-white/20 z-10"
                    >
...
/* ------------------------------------------------------------------ */
/*  Folder Tile — renders a group as a folder-style launcher icon      */
/* ------------------------------------------------------------------ */
const FolderTile = ({ group, apps, iconDims, textSize, controlProps, onClick, onDelete }: any) => {
    const members = group.memberIds
        .map((id: string) => apps.find((a: any) => a.id === id))
        .filter(Boolean)
        .slice(0, 4);

    return (
        <div
            className="flex flex-col items-center gap-4 cursor-pointer outline-none group"
            onClick={onClick}
        >
            <div
                className={`${iconDims} rounded-[2rem] bg-gradient-to-br ${group.color} shadow-lg flex items-center justify-center relative overflow-hidden hover:shadow-[0_0_50px_rgba(255,255,255,0.25)] transition-all duration-300 ring-1 ring-white/10 hover:ring-white/30`}
            >
                <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>

                {/* Inner 2x2 preview grid */}
                <div className="relative grid grid-cols-2 gap-1 p-3 w-full h-full place-content-center">
                    {[0, 1, 2, 3].map(i => {
                        const m = members[i];
                        if (!m) return <div key={i} className="aspect-square rounded-lg bg-white/10 backdrop-blur-sm"></div>;
                        const MIcon = m.icon;
                        return (
                            <div key={i} className={`aspect-square rounded-lg bg-gradient-to-br ${m.color} flex items-center justify-center shadow-sm`}>
                                {MIcon && <MIcon className="w-1/2 h-1/2 text-white" />}
                            </div>
                        );
                    })}
                </div>
                ...
                {group.memberIds.length > 0 && (
                    <div className="absolute bottom-1.5 right-1.5 bg-black/50 backdrop-blur-md text-white text-[9px] font-black px-1.5 py-0.5 rounded-md border border-white/20">
                ...
                <button
                    ...
                    className="absolute top-1.5 right-1.5 w-5 h-5 rounded-full bg-black/40 hover:bg-red-500 text-white opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"
                    title="Delete group"
                >
                    <Trash2 className="w-3 h-3" />
                </button>
            </div>
            <span className={`font-medium ${textSize} tracking-wide drop-shadow-md transition-colors ${controlProps.isDarkMode ? 'text-white/90 group-hover:text-white' : 'text-slate-600 group-hover:text-slate-900'} text-center w-28 truncate flex items-center justify-center gap-1`}>
...
/* ------------------------------------------------------------------ */
/*  Add Group Tile — launcher button to create a new group             */
/* ------------------------------------------------------------------ */
const AddGroupTile = ({ iconDims, textSize, controlProps, onClick }: any) => (
    <div
        className="flex flex-col items-center gap-4 cursor-pointer outline-none group"
        onClick={onClick}
    >
        <div className={`${iconDims} rounded-[2rem] border-2 border-dashed ${controlProps.isDarkMode ? 'border-white/20 hover:border-white/50 bg-white/5' : 'border-slate-400/40 hover:border-slate-600/60 bg-slate-200/20'} flex items-center justify-center transition-all hover:scale-105`}>
            <FolderPlus className={`w-1/3 h-1/3 ${controlProps.isDarkMode ? 'text-white/60' : 'text-slate-500'} transition-colors`} />
        </div>
        <span className={`font-medium ${textSize} tracking-wide ${controlProps.isDarkMode ? 'text-white/60' : 'text-slate-500'} text-center w-28 truncate`}>{controlProps.language === 'ar' ? 'مجموعة جديدة' : 'New Group'}</span>
    </div>
);

/* ------------------------------------------------------------------ */
/*  Folder Overlay — modal shown when a group is opened                */
/* ------------------------------------------------------------------ */
const FolderOverlay = ({ group, apps, controlProps, onClose, onLaunch, onRemove, brandTheme }: any) => {
    const members = group.memberIds
        .map((id: string) => apps.find((a: any) => a.id === id))
        .filter(Boolean);
    ...
    return (
        <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[9999] flex items-center justify-center p-6 bg-black/70 backdrop-blur-xl"
            onClick={onClose}
        >
            <motion.div
                ...
                className={`relative w-full max-w-3xl rounded-[2rem] border shadow-2xl p-8 ${controlProps.isDarkMode ? 'bg-slate-900/95 border-white/10' : 'bg-white/95 border-slate-200'}`}
            ...
            >
                ...
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className={`text-2xl font-black tracking-tight ${controlProps.isDarkMode ? 'text-white' : 'text-slate-900'}`}>{group.label}</h2>
                            <p className={`text-xs font-bold uppercase tracking-widest ${controlProps.isDarkMode ? 'text-white/50' : 'text-slate-500'}`}>{members.length} {members.length === 1 ? 'app' : 'apps'}</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                        className={`p-2.5 rounded-xl transition-all ${controlProps.isDarkMode ? 'bg-white/5 hover:bg-white/10 text-white' : 'bg-slate-100 hover:bg-slate-200 text-slate-700'}`}
                    >
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                {members.length === 0 ? (
                    <div className="py-16 text-center">
                        <Folder className={`w-12 h-12 mx-auto mb-3 ${controlProps.isDarkMode ? 'text-white/20' : 'text-slate-300'}`} />
                        <p className={`text-sm font-bold ${controlProps.isDarkMode ? 'text-white/40' : 'text-slate-400'}`}>This group is empty.</p>
                        <p className={`text-xs mt-1 ${controlProps.isDarkMode ? 'text-white/30' : 'text-slate-400'}`}>Use the ⋯ menu on any icon to move it here.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-3 md:grid-cols-4 gap-6">
                        {members.map((app: any) => {
                            ...
                            return (
                                <div key={app.id} className="flex flex-col items-center gap-3 relative group">
                                    <div
                                        className={`w-20 h-20 md:w-24 md:h-24 rounded-[1.75rem] ${!useCustomTheme ? `bg-gradient-to-br ${app.color}` : ''} shadow-lg flex items-center justify-center relative overflow-hidden cursor-pointer ring-1 ring-white/10 hover:scale-105 transition-transform`}
                                        style={iconStyle}
                                        onClick={() => { onLaunch(app.id); onClose(); }}
                                    >
                                        {Icon && <Icon className="w-1/2 h-1/2 text-white drop-shadow-md" />}
                                    </div>
                                    <span className={`text-xs font-medium text-center truncate w-24 ${controlProps.isDarkMode ? 'text-white/90' : 'text-slate-700'}`}>{app.label}</span>
                                    <button
                                        onClick={(e) => { e.stopPropagation(); onRemove(app.id); }}
                                        title="Remove from group"
                                        className="absolute -top-1 -right-1 w-6 h-6 rounded-full bg-red-500 hover:bg-red-600 text-white opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center shadow-lg"
                                    >
...
/* ------------------------------------------------------------------ */
/*  Icon Menu — the ⋯ popover attached to each launcher icon           */
/* ------------------------------------------------------------------ */
const IconMenu = ({ appId, groups, onMoveToGroup, onCreateGroup, onClose, controlProps }: any) => {
    ...
    return (
        <div
            ref={menuRef}
            className={`absolute top-2 right-2 z-[100] w-56 rounded-xl shadow-2xl border overflow-hidden ${controlProps.isDarkMode ? 'bg-slate-900/95 border-white/10 text-white' : 'bg-white/95 border-slate-200 text-slate-900'} backdrop-blur-xl`}
            onClick={(e) => e.stopPropagation()}
            onPointerDown={(e) => e.stopPropagation()}
        >
            <div className={`px-3 py-2 text-[10px] font-black uppercase tracking-widest ${controlProps.isDarkMode ? 'text-white/50' : 'text-slate-500'} border-b ${controlProps.isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
                Move to group
            </div>
            <div className="max-h-48 overflow-y-auto py-1">
                {groups.length === 0 && (
                    <div className={`px-3 py-2 text-xs ${controlProps.isDarkMode ? 'text-white/40' : 'text-slate-400'}`}>No groups yet.</div>
                )}
                {groups.map((g: any) => (
                    <button
                        key={g.id}
                        onClick={() => onMoveToGroup(g.id)}
                        className={`w-full flex items-center gap-2 px-3 py-2 text-xs font-bold text-left transition-colors ${controlProps.isDarkMode ? 'hover:bg-white/10' : 'hover:bg-slate-100'}`}
                    >
                        <div className={`w-6 h-6 rounded-md bg-gradient-to-br ${g.color} flex items-center justify-center shrink-0`}>
                            <Folder className="w-3 h-3 text-white" />
                        </div>
                        <span className="truncate flex-1">{g.label}</span>
                        <span className={`text-[10px] ${controlProps.isDarkMode ? 'text-white/40' : 'text-slate-400'}`}>{g.memberIds.length}</span>
                    </button>
                ))}
            </div>
            <button
                onClick={onCreateGroup}
                className={`w-full flex items-center gap-2 px-3 py-2 text-xs font-bold border-t ${controlProps.isDarkMode ? 'border-white/10 hover:bg-white/10 text-emerald-400' : 'border-slate-200 hover:bg-slate-100 text-emerald-600'} transition-colors`}
            >
                    ...
                    >
                        <div className="space-y-1">
                            ...
                            <div className="px-3 py-2">
                                <p className="text-[10px] uppercase font-bold tracking-wider opacity-50 mb-2">{isRtl ? 'العرض' : 'Display'}</p>
                                ...
                                <div className="space-y-1">
                                    <button onClick={() => toggleWidget('clock')} className={`w-full flex items-center justify-between px-2 py-1.5 rounded-lg text-xs font-medium transition-colors ${activeWidgets.includes('clock') ? (controlProps.isDarkMode ? 'bg-white/10 text-white' : 'bg-black/5 text-black') : 'hover:bg-black/5 opacity-70'}`}>
                                        <div className="flex items-center gap-2"><Clock className="w-3.5 h-3.5" /> {isRtl ? 'الساعة' : 'Clock'}</div>
                                    ...
                                    <button onClick={() => toggleWidget('weather')} className={`w-full flex items-center justify-between px-2 py-1.5 rounded-lg text-xs font-medium transition-colors ${activeWidgets.includes('weather') ? (controlProps.isDarkMode ? 'bg-white/10 text-white' : 'bg-black/5 text-black') : 'hover:bg-black/5 opacity-70'}`}>
                                        <div className="flex items-center gap-2"><CloudSun className="w-3.5 h-3.5" /> {isRtl ? 'الطقس' : 'Forecast'}</div>
                                    ...
                                    <button onClick={() => toggleWidget('status')} className={`w-full flex items-center justify-between px-2 py-1.5 rounded-lg text-xs font-medium transition-colors ${activeWidgets.includes('status') ? (controlProps.isDarkMode ? 'bg-white/10 text-white' : 'bg-black/5 text-black') : 'hover:bg-black/5 opacity-70'}`}>
                                        <div className="flex items-center gap-2"><CalendarDays className="w-3.5 h-3.5" /> {isRtl ? 'آخر تحديث' : 'Last Update'}</div>
                            ...
                            <button onClick={controlProps.onToggleTheme} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors hover:bg-white/10 text-main">
                                ...
                                <span className="text-sm font-medium">{controlProps.isDarkMode ? (isRtl ? 'الوضع الفاتح' : 'Light Mode') : (isRtl ? 'الوضع الداكن' : 'Dark Mode')}</span>
                            </button>
                            <button onClick={() => setIsPwdModalOpen(true)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors hover:bg-white/10 text-main">
                                ...
                                <span className="text-sm font-medium">{isRtl ? 'تغيير كلمة المرور' : 'Change Password'}</span>
                            </button>
                            {(currentUser.role === UserRole.ADMIN) && (
                                <>
                                    <button onClick={onOpenCompanySettings || controlProps.onOpenCompanySettings || (() => { })} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors hover:bg-white/10 text-main">
                                        ...
                                        <span className="text-sm font-medium">{isRtl ? 'إعدادات الشركة' : 'Company Settings'}</span>
                                    </button>
                                    <button 
                                        onClick={() => {
                                    ...
                                    >
                                        ...
                                        <span className="text-sm font-medium">{isRtl ? 'ملخص الترخيص' : 'License Summary'}</span>
                                    </button>
                                    <button 
                                        onClick={() => {
                                    ...
                                    >
                                        ...
                                        <span className="text-sm font-medium">{isRtl ? 'باقات الاشتراك' : 'Subscription Plans'}</span>
                                    </button>
                                </>
                            )}
                            ...
                            <button onClick={onLogout} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors group hover:bg-red-500/10 text-main hover:text-red-500">
                                ...
                                <span className={`text-sm font-medium ${isRtl ? 'mr-2' : ''}`}>{isRtl ? 'تسجيل الخروج' : 'Log Out'}</span>
                            </button>
                        </div>
                    </motion.div>
                )}
...
const ModernOSLayout: React.FC<ModernLayoutProps> = (props) => {
    ...
    const STORAGE_KEYS = {
        ...
        GROUPS: `modern_app_groups_${currentUser?.id || 'guest'}`
    };
    ...
    const allApps = React.useMemo(() => [
        ...
        {
            id: ViewMode.INSIGHTS_V2,
            label: isRtl ? 'التحليلات V2' : 'Insights V2',
            icon: Activity,
            color: 'from-emerald-500 via-teal-500 to-cyan-500',
            enabled: props.currentCompany?.settings?.modules?.insights?.enabled !== false
        },
        {
            id: ViewMode.AI_OPTIMIZER_V2,
        ...
        {
            id: ViewMode.DATA_ASSIST_V2,
            label: isRtl ? 'مساعد البيانات' : 'Data Assist',
            icon: Sparkles,
            color: 'from-emerald-400 via-cyan-500 to-teal-500',
            enabled: true
        },
        {
            id: ViewMode.SMART_WORKFLOWS,
            label: isRtl ? 'المهام الذكية' : 'Smart Workflows',
            icon: Workflow,
        ...
        {
            id: ViewMode.DETAILED_REPORTS_V2,
            label: (controlProps.language === 'ar' ? 'التقارير المفصلة' : 'Detailed Reports') + ' V2',
            icon: ClipboardList,
        ...
        },
        {
            ...
            label: isRtl ? 'تحليل بياناتي' : 'Analyze Own Data',
            icon: BarChart3,
        ...
        {
            id: ViewMode.COMPANY_SETTINGS_V2,
            label: t.settings,
            icon: Settings,
            color: 'from-cyan-500 via-purple-500 to-pink-500',
            enabled: currentUser.role === UserRole.ADMIN
    ...
    // Effect: Sync isHome with the active view.
    // - DASHBOARD = the icon launcher → ensure isHome is true (used by every "back" arrow).
    // - REFERRAL_HUB and other non-launcher views → ensure isHome is false.
    useEffect(() => {
        if (view === ViewMode.DASHBOARD) {
            setIsHome(true);
            localStorage.setItem(`modern_is_home_${currentUser?.id || 'guest'}`, 'true');
        } else if (view === ViewMode.REFERRAL_HUB) {
            setIsHome(false);
            localStorage.setItem(`modern_is_home_${currentUser?.id || 'guest'}`, 'false');
        }
    }, [view, currentUser]);
    ...
    // --- APP GROUPS (folders) ---
    // Icons available for group labels. Keyed by string so groups can persist.
    const GROUP_ICON_MAP: Record<string, React.ElementType> = {
        Wrench, Folder, FolderOpen, Sparkles, Radar, ShieldCheck, Navigation
    };
    ...
    interface AppGroup {
        ...
        iconName: keyof typeof GROUP_ICON_MAP | string;
        memberIds: string[];
    }
    ...
    const DEFAULT_GROUPS: AppGroup[] = [
        {
            ...
            label: isRtl ? 'قيد التطوير' : 'Under Development',
            color: 'from-amber-500 via-orange-500 to-red-500',
            iconName: 'Wrench',
            memberIds: [ViewMode.INSIGHTS_V2, ViewMode.DATA_ASSIST_V2, ViewMode.SMART_WORKFLOWS, ViewMode.DETAILED_REPORTS_V2]
        }
    ];
    ...
    const memberIdSet = React.useMemo(() => {
        ...
        groups.forEach(g => g.memberIds.forEach(id => s.add(id)));
        return s;
    }, [groups]);

    const [openGroupId, setOpenGroupId] = useState<string | null>(null);
    const [iconMenuId, setIconMenuId] = useState<string | null>(null); // which icon has its ⋯ menu open

    const handleMoveToGroup = (appId: string, groupId: string) => {
        setGroups(prev => prev.map(g => {
            if (g.id === groupId) {
                if (g.memberIds.includes(appId)) return g;
                return { ...g, memberIds: [...g.memberIds, appId] };
            }
            // also remove from any other group
            if (g.memberIds.includes(appId)) {
                return { ...g, memberIds: g.memberIds.filter(id => id !== appId) };
            }
            return g;
        }));
        setIconMenuId(null);
    ...
    const handleRemoveFromGroup = (appId: string, groupId: string) => {
        setGroups(prev => prev.map(g =>
            g.id === groupId ? { ...g, memberIds: g.memberIds.filter(id => id !== appId) } : g
        ));
    };
    ...
    const handleDeleteGroup = (groupId: string) => {
        if (!window.confirm('Delete this group? Its icons return to the main screen.')) return;
                                        ...
                                        >
                                            ...
                                            {activeWidgets.includes('weather') && (
                                                <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className="p-6 rounded-3xl border border-main flex items-center gap-4 shadow-xl glass-panel text-main">
                                                    ...
                                                    <div>
                                                        <div className="text-sm font-bold opacity-60">{isRtl ? 'الطقس' : 'Forecast'}</div>
                                                        <div className="text-xs font-medium text-muted">{isRtl ? 'البيانات المباشرة غير متاحة' : 'Live data not configured'}</div>
                                                    </div>
                                                </motion.div>
                                            )}
                                            {activeWidgets.includes('status') && (
                                                <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className="p-6 rounded-3xl border border-main flex items-center gap-4 shadow-xl glass-panel text-main">
                                                    ...
                                                    <div>
                                                        <div className="text-sm font-bold">{isRtl ? 'آخر تحديث' : 'Last Updated'}</div>
                                                        <div className="flex justify-between items-center text-xs font-bold text-main mb-2">
                                                            <div className="flex items-center gap-2"><Clock className="w-3 h-3 text-cyan-400" /> {lastUpload ? new Date(lastUpload.uploadDate).toLocaleDateString() : (isRtl ? 'لا توجد بيانات' : 'No Data')}</div>
                                                            <div className="flex items-center gap-2"><User className="w-3 h-3 text-cyan-400" /> {lastUpload ? lastUpload.uploader : (isRtl ? 'لا توجد بيانات' : 'No Data')}</div>
                                                        </div>
                                                    </div>
                                                </motion.div>
                                            )}
                                        ...
                                        >
                                            ...
                                            >
                                                {appsOrder.map(appId => {
                                                    ...
                                                    // Hide apps that live inside a group — they appear inside the folder instead
                                                    if (memberIdSet.has(appId)) return null;
                                                    const isMenuOpen = iconMenuId === appId;
                                                    return (
                                                        <SortableItem
                                                            key={appId}
                                                            id={appId}
                                                            ...
                                                            onOpenMenu={() => setIconMenuId(prev => prev === appId ? null : appId)}
                                                            menuOpen={isMenuOpen}
                                                            menuSlot={isMenuOpen && (
                                                                <IconMenu
                                                                    appId={appId}
                                                                    groups={groups}
                                                                    controlProps={controlProps}
                                                                    onMoveToGroup={(gid: string) => handleMoveToGroup(appId, gid)}
                                                                    onCreateGroup={() => {
                                                                        handleCreateGroup();
                                                                        setIconMenuId(null);
                                                                    }}
                                                                    onClose={() => setIconMenuId(null)}
                                                                />
                                                            )}
                                                        />
                                                    );
                                ...
                                {openGroupId && (() => {
                                    ...
                                    return (
                                        <FolderOverlay
                                            ...
                                            onLaunch={(id: string) => handleAppLaunch(id)}
                                            onRemove={(id: string) => handleRemoveFromGroup(id, group.id)}
                                        />
                                    );
                                })()}
                            </AnimatePresence>
