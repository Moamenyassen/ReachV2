...
const AppContent: React.FC<AppContentProps> = (props) => {
    ...
    return (
        <>
            ...
            {!isLimbo && view === ViewMode.INSIGHTS_V2 && <InsightsV2 currentUser={currentUser} currentCompany={currentCompany} allCustomers={allCustomers} userList={users} uploadHistory={uploadHistory} onNavigate={setView} onLogout={onLogout} hideHeader={props.hideHeader} {...controlProps} />}
            {!isLimbo && (view === ViewMode.DASHBOARD || view === ViewMode.ROUTE_SEQUENCE_V2) && <RouteSequenceV2 onBack={() => setView(ViewMode.DASHBOARD)} settings={currentCompany?.settings || DEFAULT_COMPANY_SETTINGS} companyId={currentCompany?.id} activeVersionId={activeVersionId} userRole={currentUser?.role} userBranchIds={currentUser?.branchIds} lastUploadDate={uploadHistory?.[0]?.uploadDate} {...controlProps} hideHeader={props.hideHeader} />}
            {!isLimbo && view === ViewMode.FULL_SUMMARY && <DetailedReports currentUser={currentUser} allCustomers={accessibleCustomers} uploadHistory={uploadHistory} onBack={() => setView(ViewMode.DASHBOARD)} {...controlProps} hideHeader={props.hideHeader} currentFilters={props.currentFilters} />}
            {!isLimbo && view === ViewMode.DETAILED_REPORTS_V2 && <DetailedReportsV2 currentUser={currentUser} onBack={() => setView(ViewMode.DASHBOARD)} hideHeader={props.hideHeader} currentFilters={props.currentFilters} {...controlProps} />}
            {!isLimbo && (view === ViewMode.AI_SUGGESTIONS || view === ViewMode.AI_OPTIMIZER_V2) && <AIOptimizerV2 onBack={() => setView(ViewMode.DASHBOARD)} {...controlProps} hideHeader={props.hideHeader} companyId={currentCompany?.id} userBranchIds={currentUser?.branchIds} userRole={currentUser?.role} />}
            {!isLimbo && (view === ViewMode.MARKET_SCANNER || view === ViewMode.MARKET_SCANNER_V2) && <MarketScannerV2 existingCustomers={accessibleCustomers} onBack={() => setView(ViewMode.DASHBOARD)} settings={currentCompany?.settings || DEFAULT_COMPANY_SETTINGS} maxScannerCap={currentCompany?.maxScannerCap} companyId={currentCompany?.id} userRole={currentUser?.role} userBranchIds={currentUser?.branchIds} {...controlProps} hideHeader={props.hideHeader} />}
            {!isLimbo && view === ViewMode.COMPANY_SETTINGS_V2 && currentCompany && <CompanySettingsV2 company={currentCompany} onClose={() => setView(ViewMode.DASHBOARD)} currentUserRole={currentUser?.role} />}
            {!isLimbo && view === ViewMode.DATA_ASSIST_V2 && <DataAssistV2 companyId={currentCompany?.id} userId={currentUser?.id} userBranchIds={currentUser?.branchIds} onBack={() => setView(ViewMode.DASHBOARD)} hideHeader={props.hideHeader} {...controlProps} />}
            {!isLimbo && view === ViewMode.SMART_WORKFLOWS && <SmartWorkflows companyId={currentCompany?.id} userId={currentUser?.id} onBack={() => setView(ViewMode.DASHBOARD)} hideHeader={props.hideHeader} {...controlProps} />}
            {!isLimbo && (currentUser.role === UserRole.ADMIN) && view === ViewMode.PRICING && <Pricing onBack={() => setView(ViewMode.DASHBOARD)} hideHeader={props.hideHeader} isAiTheme={controlProps.isAiTheme} onSubscribe={(plan) => onSubscribe(plan, 'monthly')} />}
