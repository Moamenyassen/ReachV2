...
const MARKER_STYLES = `
  ...
  .reach-pin.swap-candidate { z-index: 950 !important; animation: swap-pulse 2s ease-in-out infinite; }
  @keyframes swap-pulse { 0%,100% { filter: drop-shadow(0 0 4px rgba(251,146,60,0.6)); } 50% { filter: drop-shadow(0 0 10px rgba(251,146,60,1)); } }
...
const getSwapIcon = (num: number, selected: boolean): L.DivIcon => {
  const key = `swap-${num}-${selected ? 1 : 0}`;
  if (iconCache.has(key)) return iconCache.get(key)!;
  const html = `
    <div style="position:relative;width:36px;height:48px;">
      <svg width="36" height="48" viewBox="0 0 36 48" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M18 0C8.06 0 0 8.06 0 18C0 31 16 47 18 48C20 47 36 31 36 18C36 8.06 27.94 0 18 0Z" fill="#f97316"/>
        <circle cx="18" cy="18" r="13" fill="white"/>
        <circle cx="18" cy="18" r="11" fill="#fff7ed"/>
      </svg>
      <div style="position:absolute;top:0;left:0;width:36px;height:36px;display:flex;align-items:center;justify-content:center;font-weight:800;font-family:Inter,sans-serif;font-size:${num > 99 ? 8 : num > 9 ? 10 : 11}px;color:#ea580c;">
        ${num}
      </div>
      <div style="position:absolute;top:-5px;right:-5px;width:14px;height:14px;background:#f97316;border-radius:50%;border:2px solid white;display:flex;align-items:center;justify-content:center;font-size:8px;">✦</div>
    </div>
  `;
  const icon = L.divIcon({
    className: `reach-pin swap-candidate ${selected ? 'selected' : ''}`,
    html,
    iconSize: [36, 48],
    iconAnchor: [18, 48],
    popupAnchor: [0, -44],
      ...
      >
        ...
        {allMarkers
          ...
          .map((customer, idx) => {
            ...
            const isSwap = !isBranch && !!swapCustomerIds?.has(String(customer.id));
            ...
            if (isBranch) {
              ...
              icon = getSwapIcon(customerIdx, isSelected);
            } else if (isHeavy) {
              ...
              >
                <Popup closeButton={false} className="custom-popup" offset={[0, 6]}>
                  <div className="font-sans">
                    ...
                    {/* Swap badge */}
                    {isSwap && (
                      <div style={{ background: '#fff7ed', borderBottom: '1px solid #fed7aa', padding: '6px 12px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <span style={{ fontSize: '10px' }}>✦</span>
                        <span style={{ fontSize: '10px', fontWeight: 800, color: '#ea580c', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Smart Swap Suggested</span>
                      </div>
                    )}
                    {/* Body */}
                    <div className="p-3">
