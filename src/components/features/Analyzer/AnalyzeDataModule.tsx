...
const AnalyzeDataModule: React.FC<{ companyId: string, userId: string }> = ({ companyId, userId }) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setChatHistory(prev => [...prev, { role: 'assistant', content: `⚠️ ${err?.message || 'Sorry, I could not process that. Please try again.'}` }]);
    } finally {
        ...
        {status === 'error' && (
          ...
          >
            ...
            <p className="text-slate-500 text-sm mb-8">
              Make sure <code className="text-cyan-400 bg-white/5 px-1 rounded">GEMINI_API_KEY</code> (or <code className="text-cyan-400 bg-white/5 px-1 rounded">GOOGLE_API_KEY</code>) is set in <code className="text-cyan-400 bg-white/5 px-1 rounded">server_py/.env</code>
            </p>
            {errorDetail && (
              <p className="text-red-300 text-sm mb-8 whitespace-pre-wrap break-words">
                {errorDetail}
              </p>
            )}
            <button
