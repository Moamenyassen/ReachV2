...
const Customers: React.FC<CustomersProps> = ({
    ...
    const renderCell = (customer: Customer, field: keyof Customer, width: string = 'w-32', placeholder: string = '') => {
        ...
        return <div className={`truncate ${width}`} title={String(value || '')}>{typeof value === 'object' ? JSON.stringify(value) : (value || <span className="text-slate-400 italic">{isAr ? 'فارغ' : 'Empty'}</span>)}</div>;
    };
    ...
    if (isEmpty && !isGlobalLoading) {
        return (
            <div className="h-full flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-500">
                ...
                <h2 className="2xl font-black text-slate-800 dark:text-white mb-2">{isAr ? 'لا يوجد عملاء' : 'No Customers Found'}</h2>
                <div className="flex gap-4 mt-8">
                    {onUpload && (<button onClick={onUpload} className="px-6 py-3 bg-indigo-600 text-white rounded-xl font-bold">{isAr ? 'رفع البيانات' : 'Upload Data'}</button>)}
                </div>
            </div>
        );
    }
    ...
    if (hasNoAccess) {
        return (
            <div className="h-full flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-500 bg-slate-50 dark:bg-[#0f172a]">
                ...
                <h2 className="text-2xl font-black text-slate-800 dark:text-white mb-2">{isAr ? 'الوصول مقيّد' : 'Access Restricted'}</h2>
                <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto">
                    {isAr ? 'تواصل مع مدير النظام لتعيين الفروع لحسابك. لا تملك حالياً صلاحية عرض بيانات العملاء.' : 'Contact your system administrator to get branches assigned to your account. You currently do not have access to view any customer data.'}
                </p>
                <div className="mt-8">
            ...
            {!hideHeader && (
                <div className="shrink-0 p-6 border-b border-slate-200 dark:border-white/5 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <div className="flex items-center gap-3">
                                <h1 className="text-xl font-black text-slate-900 dark:text-white">{isAr ? 'قاعدة بيانات العملاء' : 'Customer Database'}</h1>
                                <div className="flex items-center gap-2 px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                                    ...
                                    <span className="text-[10px] uppercase tracking-wider text-slate-500 font-medium">{isAr ? 'سجل فريد' : 'Distinct Records'}</span>
                                </div>
                            </div>
                            <div className="flex items-center gap-2">
                                {isFetching && (
                                    <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-indigo-500/10 border border-indigo-500/20">
                                        ...
                                        <span className="text-[9px] font-bold text-indigo-500 uppercase tracking-tighter">{isAr ? 'جارٍ التحميل...' : 'Loading...'}</span>
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            <div className={`shrink-0 px-6 py-3 border-b ${isDarkMode ? 'border-slate-800 bg-slate-900/50' : 'border-slate-200 bg-white'} flex flex-col gap-4`}>
                ...
                <div className="flex flex-col md:flex-row items-center gap-4">
                    ...
                    <div className="relative flex-1 w-full">
                        ...
                        <input
                            type="text"
                            placeholder={isAr ? 'ابحث بالاسم أو الكود أو العنوان... (اضغط Enter للبحث)' : 'Search by name, code, or address... (Enter to search)'}
                            value={searchTerm}
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        >
                            ...
                            <span className="text-xs font-bold">{isAr ? 'الفلاتر' : 'Filters'}</span>
                    ...
                    {isFiltersOpen && (
                        <div className="flex items-center gap-2 w-full overflow-x-auto p-2 bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-lg animate-in slide-in-from-top-2 duration-200">
                            ...
                            >
                                ...
                                <option value="Missing GPS">⚠️ {isAr ? 'GPS مفقود' : 'Missing GPS'}</option>
                                <option value="Nearby" disabled>🎯 {isAr ? 'فرع قريب (غير متاح)' : 'Nearby Branch (N/A)'}</option>
                            </select>
                            ...
                            >
                                ...
                                <option value="Scanner">📡 {isAr ? 'من الرادار' : 'From Scanner'}</option>
                            </select>
                    ...
                    {onUpload && (
                        <div className="pl-4 md:border-l border-slate-200 dark:border-slate-700 ml-auto md:ml-0">
                            ...
                            >
                                ...
                                <span className="hidden md:inline">{isAr ? 'رفع البيانات' : 'Upload Data'}</span>
                                <span className="md:hidden">{isAr ? 'رفع' : 'Upload'}</span>
                            </button>
                        </div>
                    )}
                </div>
                ...
                <div className={`w-full h-full overflow-auto custom-scrollbar rounded-xl border ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
                    <table className="min-w-max w-full text-left text-xs">
                        <thead className={`${isDarkMode ? 'bg-slate-800/80 text-slate-400' : 'bg-slate-50 text-slate-500'} font-bold uppercase tracking-wider sticky top-0 z-10 backdrop-blur-md`}>
                            <tr>
                                {[
                                    { label: isAr ? 'الكود' : 'Code', key: 'clientCode', width: 'w-24' },
                                    { label: isAr ? 'الاسم' : 'Name', key: 'name', width: 'w-48' },
                                    { label: isAr ? 'الاسم (ع)' : 'Name (AR)', key: 'nameAr', width: 'w-32' },
                                    { label: isAr ? 'الفرع' : 'Branch', key: 'branch', width: 'w-32' },
                                    { label: isAr ? 'الهاتف' : 'Phone', key: 'phone', width: 'w-32' },
                                    { label: isAr ? 'العنوان' : 'Address', key: 'address', width: 'w-64' },
                                    { label: isAr ? 'الحي' : 'District', key: 'district', width: 'w-32' },
                                    { label: isAr ? 'الضريبة' : 'VAT', key: 'vat', width: 'w-32' },
                                    { label: isAr ? 'رقم المشتري' : 'Buyer ID', key: 'buyerId', width: 'w-32' },
                                    { label: isAr ? 'التصنيف' : 'Class', key: 'classification', width: 'w-24' },
                                    { label: isAr ? 'النوع' : 'Type', key: 'storeType', width: 'w-24' },
                                    { label: 'GPS', key: 'lat', width: 'w-32' },
                                    { label: isAr ? 'المنطقة' : 'Region', key: 'regionDescription', width: 'w-32' },
                                    { label: isAr ? 'كود المنطقة' : 'Reg. Code', key: 'regionCode', width: 'w-24' },
                                    { label: isAr ? 'المسار' : 'Route', key: 'routeName', width: 'w-32' },
                                    { label: isAr ? 'كود Reach' : 'Reach Code', key: 'reachCustomerCode', width: 'w-32' },
                                    { label: isAr ? 'اليوم' : 'Day', key: 'day', width: 'w-24' },
                                    { label: isAr ? 'الأسبوع' : 'Week', key: 'week', width: 'w-24' },
                                    { label: isAr ? 'أضيف بواسطة' : 'Added By', key: 'addedBy', width: 'w-32' },
                                    { label: isAr ? 'التاريخ' : 'Date', key: 'addedDate', width: 'w-32' },
                                ].map(col => (
                                    <th
                                        key={col.key}
                                ...
                                <th className="px-4 py-3 text-right sticky right-0 bg-inherit shadow-[-10px_0_10px_-10px_rgba(0,0,0,0.1)]">{isAr ? 'إجراء' : 'Action'}</th>
                            </tr>
                        </thead>
