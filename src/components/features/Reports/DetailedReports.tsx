...
const DetailedReports: React.FC<DetailedReportsProps> = ({
  ...
  const tabs = [
    { id: 'tab1', label: isAr ? 'العرض الهرمي'    : 'Hierarchical View',  Component: HierarchyTab },
    { id: 'tab2', label: isAr ? 'ملخص المسارات'   : 'Route Summary',       Component: RouteSummaryTab },
    { id: 'tab3', label: isAr ? 'تكرار الزيارات'  : 'Visit Frequency',     Component: VisitFrequencyTab },
    { id: 'tab4', label: isAr ? 'كفاءة المسارات'  : 'Route Efficiency',    Component: RouteEfficiencyTab },
    { id: 'tab5', label: isAr ? 'عبء المندوبين'   : 'User Workload',       Component: UserWorkloadTab },
    { id: 'tab6', label: isAr ? 'جودة البيانات'   : 'Data Quality',        Component: DataQualityTab },
    { id: 'tab7', label: isAr ? 'التغطية الأسبوعية': 'Weekly Coverage',    Component: WeeklyCoverageTab },
  ];

  return (
    <div data-reach-screen className="min-h-screen bg-gray-950 flex flex-col font-sans relative z-10 text-gray-100">
      <FilterSection
        filters={filters}
      ...
      <div className="bg-gray-900/50 border-b border-gray-800 px-6 py-2 flex items-center gap-4 text-xs">
        ...
        {loading && (
          <span className="flex items-center gap-1 text-cyan-400">
            <Loader2 size={12} className="animate-spin" /> {isAr ? 'جارٍ التحميل…' : 'Loading…'}
          </span>
        )}
        {!loading && !error && (
          <span className="text-gray-400">
            <span className="text-emerald-400 font-bold">{rawData.length}</span> {isAr ? 'صف محمّل' : 'rows loaded'}
            {filters.region && filters.region !== 'All' && (
              <> · <span className="text-cyan-400 font-bold">{filteredData.length}</span> {isAr ? 'بعد فلتر المنطقة' : 'after region filter'}</>
            )}
          </span>
        )}
        {error && (
