...
if (!SUPABASE_URL || !SUPABASE_KEY) {
    // Fail loudly — no hardcoded fallback. The anon key must come from .env.local
    // so it can be rotated without a code release.
    throw new Error('[Security] VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY must be set in .env.local.');
}
...
export const provisionDemoCompany = async (
    ...
    try {
        ...
        // 4. Link original registered user to the new company (promote to admin)
        // Try linked_user_id first, then fall back to email lookup
        let linkedUserId: string | null = null;
        if (lead.linked_user_id) {
            const { data: byId } = await supabase.from('app_users').select('id').eq('id', lead.linked_user_id).maybeSingle();
            if (byId) linkedUserId = byId.id;
        }
        if (!linkedUserId && lead.email) {
            const { data: byEmail } = await supabase.from('app_users').select('id').eq('email', lead.email).maybeSingle();
        ...
        if (linkedUserId) {
            const { error: linkErr } = await supabase
                .from('app_users')
                .update({ company_id: companyId, role: UserRole.ADMIN, is_active: true })
                .eq('id', linkedUserId);
            if (linkErr) {
                console.error('[provisionDemoCompany] Failed to link user → falling back to new admin user:', linkErr);
            }
        }

        // If no existing user was linked, create a standalone admin user
        if (!linkedUserId) {
            await addGlobalUser({
...
// --- Route Health Details from company_uploaded_data ---
// Matches the same source the dashboard donut uses (RPC reads company_uploaded_data),
// so the per-route list reconciles with the totals shown in Insights.
export const fetchRouteHealthFromUploads = async (
    companyId: string,
    branchNames?: string[],
    thresholds?: { min?: number; max?: number }
) => {
    const min = thresholds?.min ?? 80;
    const max = thresholds?.max ?? 120;

    const rows = await fetchAllUploadedRows<{
        client_code: string | null;
        route_name: string | null;
        branch_name: string | null;
    }>(companyId, 'client_code, route_name, branch_name', branchNames);

    const groups: Record<string, { branch: string; region: string; clients: Set<string>; rows: number }> = {};
    rows.forEach((r) => {
        const routeName = r.route_name || 'Unassigned';
        if (routeName === 'Unassigned' || !routeName) return;
        if (!groups[routeName]) {
            groups[routeName] = {
                branch: r.branch_name || '—',
                region: r.branch_name || '—',
                clients: new Set(),
                rows: 0,
            };
        }
        groups[routeName].rows += 1;
        const key = r.client_code || `${routeName}:${groups[routeName].rows}`;
        groups[routeName].clients.add(key);
    });

    const details: any[] = [];
    let healthy = 0, under = 0, over = 0;
    Object.entries(groups).forEach(([routeName, g]) => {
        const distinctCount = g.clients.size;
        let status: 'stable' | 'under' | 'over';
        let efficiency: number;
        if (distinctCount < min) { status = 'under'; under++; efficiency = Math.round(50 + (distinctCount / Math.max(min, 1)) * 30); }
        else if (distinctCount > max) { status = 'over'; over++; efficiency = Math.round(95 - Math.min(25, ((distinctCount - max) / Math.max(max, 1)) * 100)); }
        else { status = 'stable'; healthy++; efficiency = Math.round(85 + ((distinctCount - min) / Math.max(max - min, 1)) * 10); }
        details.push({
            routeName,
            branchName: g.branch,
            region: g.region,
    ...
    return { healthy, under, over, total: healthy + under + over, details };
};

// Parallel pagination through company_uploaded_data.
// Bypasses Supabase's default 1000-row cap by fetching every page in
// concurrent batches once we know the total row count.
const PAGE_SIZE = 1000;
const FETCH_CONCURRENCY = 8;

const fetchAllUploadedRows = async <T>(
    companyId: string,
    columns: string,
    branchNames?: string[]
): Promise<T[]> => {
    ...
    let countQ = supabase
        ...
        .eq('company_id', companyId);
    if (branchNames && branchNames.length > 0) countQ = countQ.in('branch_name', branchNames);
    const { count, error: countErr } = await countQ;
    if (countErr) {
        console.error('[fetchAllUploadedRows] count error:', countErr);
        return [];
    }
    const total = count ?? 0;
    if (total === 0) return [];
    // Hard cap so a misconfigured query can't run away
    if (total > 500000) {
        console.warn('[fetchAllUploadedRows] count exceeds 500k cap, truncating', total);
    }
    const effective = Math.min(total, 500000);
    const numPages = Math.ceil(effective / PAGE_SIZE);

    // 2. Page fetcher
    const fetchPage = async (idx: number): Promise<T[]> => {
        const from = idx * PAGE_SIZE;
        let q = supabase
            .from('company_uploaded_data')
            .select(columns)
            .eq('company_id', companyId)
            .range(from, from + PAGE_SIZE - 1);
        if (branchNames && branchNames.length > 0) q = q.in('branch_name', branchNames);
        const { data, error } = await q;
        if (error) {
            console.error('[fetchAllUploadedRows] page', idx, 'error:', error);
            return [];
        }
        return (data || []) as T[];
    };
    ...
    const all: T[] = [];
    for (let cursor = 0; cursor < numPages; cursor += FETCH_CONCURRENCY) {
        const batch = Array.from({ length: Math.min(FETCH_CONCURRENCY, numPages - cursor) }, (_, i) => cursor + i);
...
// --- Branch Stats from company_uploaded_data ---
// Returns per-branch route count + distinct customer count for the Insights map popup.
// Keys are stored UPPERCASE so callers can do case-insensitive lookups.
export const fetchBranchStatsFromUploads = async (
    companyId: string,
    branchNames?: string[]
): Promise<Record<string, { routes: number; customers: number }>> => {
    const rows = await fetchAllUploadedRows<{ branch_name: string | null; route_name: string | null; client_code: string | null }>(
        companyId,
        'branch_name, route_name, client_code',
        branchNames,
    );

    const groups: Record<string, { routes: Set<string>; clients: Set<string> }> = {};
    rows.forEach((r) => {
        const branch = (r.branch_name || '').trim();
        if (!branch) return;
        const key = branch.toUpperCase();
        if (!groups[key]) groups[key] = { routes: new Set(), clients: new Set() };
        if (r.route_name) groups[key].routes.add(r.route_name);
        if (r.client_code) groups[key].clients.add(r.client_code);
    });

    const result: Record<string, { routes: number; customers: number }> = {};
    Object.entries(groups).forEach(([name, g]) => {
        result[name] = { routes: g.routes.size, customers: g.clients.size };
    });
    return result;
};

// --- Data Alert Details from company_uploaded_data ---
// Powers the "Missing GPS" / "Proximity Issues" popup tables in Insights.
export interface DataAlertRow {
    ...
    lat: number | null;
    lng: number | null;
    distanceMeters?: number | null; // proximity-only
}
...
export const fetchMissingGpsRows = async (
    companyId: string,
    branchNames?: string[]
): Promise<DataAlertRow[]> => {
    const rows = await fetchAllUploadedRows<{
        client_code: string | null; customer_name_en: string | null; customer_name_ar: string | null;
        branch_name: string | null; route_name: string | null; district: string | null;
        lat: number | null; lng: number | null;
    }>(companyId, 'client_code, customer_name_en, customer_name_ar, branch_name, route_name, district, lat, lng', branchNames);

    // Dedup by client_code so we don't list the same customer multiple times across visits
    const seen = new Set<string>();
    const out: DataAlertRow[] = [];
    rows.forEach(r => {
        const missing = r.lat === null || r.lng === null || r.lat === 0 || r.lng === 0;
        if (!missing) return;
        const key = r.client_code || `${r.customer_name_en || ''}@${r.branch_name || ''}@${r.route_name || ''}`;
        if (seen.has(key)) return;
        seen.add(key);
        out.push({
            clientCode: r.client_code,
            customerName: r.customer_name_en || r.customer_name_ar,
...
export const fetchProximityIssueRows = async (
    companyId: string,
    radiusMeters: number = 100,
    branchNames?: string[]
): Promise<DataAlertRow[]> => {
    // Need branch coords to compute distances
    const { data: branches, error: bErr } = await supabase
        .from('company_branches')
        .select('name_en, code, lat, lng')
        .eq('company_id', companyId);
    if (bErr) console.error('[fetchProximityIssueRows] branches error', bErr);
    const branchMap = new Map<string, { lat: number; lng: number; name: string }>();
    (branches || []).forEach((b: any) => {
        if (b.lat == null || b.lng == null) return;
        const key = (b.name_en || '').toUpperCase();
        if (key) branchMap.set(key, { lat: b.lat, lng: b.lng, name: b.name_en });
        if (b.code) branchMap.set(String(b.code).toUpperCase(), { lat: b.lat, lng: b.lng, name: b.name_en });
    });
    ...
    const out: DataAlertRow[] = [];
    rows.forEach(r => {
        if (r.lat == null || r.lng == null || r.lat === 0 || r.lng === 0) return;
        const branchKey = (r.branch_name || '').toUpperCase();
        const branch = branchMap.get(branchKey);
        if (!branch) return;
        const dist = haversineMeters({ lat: r.lat, lng: r.lng }, { lat: branch.lat, lng: branch.lng });
        if (dist > radiusMeters) return;
        const key = r.client_code || `${r.customer_name_en || ''}@${r.branch_name || ''}`;
        if (seen.has(key)) return;
        seen.add(key);
        out.push({
            clientCode: r.client_code,
    ...
    // Closest first
    out.sort((a, b) => (a.distanceMeters ?? Infinity) - (b.distanceMeters ?? Infinity));
    return out;
};
