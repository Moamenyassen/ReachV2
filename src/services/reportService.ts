...
// ─────────────────────────────────────────────
// Central fetch: parallel pagination + minimal columns
//
// Performance notes:
//  - select(REPORT_COLUMNS) instead of '*' shrinks payload ~4x (skips heavy
//    JSON/text columns like address, vat, name_ar, store_type metadata).
//  - We fetch the row count first, then fire all pages concurrently in chunks
//    of CONCURRENCY (capped to avoid overwhelming Postgres). 45 sequential
//    1s round-trips → ~5 parallel batches in the same time.
//  - First call still does the work; subsequent calls are served by React
//    Query's cache (5 min staleTime in the consumer).
// ─────────────────────────────────────────────
const REPORT_COLUMNS = [
    'id',
    'client_code', 'customer_name_en',
    'lat', 'lng',
    'branch_name', 'branch_code',
    'route_name',
    'rep_code',
    'classification', 'store_type', 'district',
    'phone', 'vat',
    'week_number', 'day_name',
].join(', ');

const PAGE = 1000;
const CONCURRENCY = 8; // simultaneous in-flight page requests
...
export const fetchReportData = async (
    ...
    // 1. Get exact row count (cheap — head:true sends no body)
    let countQ = supabase
        .from('company_uploaded_data')
        .select('id', { count: 'exact', head: true })
        .eq('company_id', companyId);
    if (branchFilter && branchFilter.length > 0) {
        countQ = countQ.in('branch_name', branchFilter);
    }
    const { count: totalCount, error: countErr } = await countQ;
    if (countErr) {
        console.error('[reportService] count error:', countErr);
        throw new Error(`Reports count failed: ${countErr.message}`);
    }
    const total = totalCount ?? 0;
    if (total === 0) return [];

    const numPages = Math.ceil(total / PAGE);
    console.log(`[reportService] Fetching ${total} rows in ${numPages} pages × ${CONCURRENCY} concurrent`);

    // 2. Fire pages in parallel batches
    const fetchPage = async (pageIdx: number): Promise<RawRow[]> => {
        const fromRow = pageIdx * PAGE;
        let q = supabase
            .from('company_uploaded_data')
        ...
        if (error) throw new Error(`page ${pageIdx} failed: ${error.message}`);
        return (data || []) as unknown as RawRow[];
    };

    const all: RawRow[] = new Array(total);
    let cursor = 0;
    let nextSlot = 0;
    const pageIndices = Array.from({ length: numPages }, (_, i) => i);

    while (cursor < pageIndices.length) {
        const batch = pageIndices.slice(cursor, cursor + CONCURRENCY);
        cursor += CONCURRENCY;
    ...
    // Some pages may return slightly less than PAGE on the last page; trim
    const trimmed = all.slice(0, nextSlot);
    console.log(`[reportService] Fetched ${trimmed.length} raw rows`);

    // Deduplicate by client_code + route + week + day (one row per visit)
    const seen = new Set<string>();
    const unique: RawRow[] = [];
    for (const r of trimmed) {
        const key = `${r.client_code || r.id}_${r.route_name || ''}_${r.week_number || ''}_${r.day_name || ''}`;
